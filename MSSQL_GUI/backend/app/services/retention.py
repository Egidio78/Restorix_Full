...
def _safe_norm(p: str | None) -> str | None:
    """Best-effort normalize: never raises (used in audit metadata)."""
    if not p:
        return p
    try:
        return normalize_remote_path(p)
    except Exception:
        return p
        ...
        try:
            ...
            try:
                await log_event(
                    ...
                    metadata={"file_path": _safe_norm(run.file_path), "storage_type": storage_type},
                )
                ...
                try:
                    await log_event(
                        ...
                        metadata={
                            ...
                            "file_path": _safe_norm(run.file_path),
                        },
                    )
                except Exception as audit_exc:
                    logger.warning("audit log failed: %s", audit_exc)
