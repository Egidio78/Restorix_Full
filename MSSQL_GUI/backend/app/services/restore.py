...
from app.core.paths import normalize_basename
...
class RestoreService:
    ...
    def _validate_temp_dir(self, raw: str) -> Path:
        if not raw:
            raise HTTPException(
                status_code=400,
                detail="restore_temp_dir non configurato",
            )
        if ".." in raw:
            raise HTTPException(
                status_code=400,
                detail=f"restore_temp_dir non puo' contenere '..': {raw!r}",
            )
        if not raw.startswith("/"):
            raise HTTPException(
                status_code=400,
                detail=(
                    f"restore_temp_dir deve essere un path assoluto Linux "
                    f"(es. /var/lib/restorix/). Il backend gira in container Linux, "
                    f"anche se gli agenti possono essere Windows. Ricevuto: {raw!r}"
                ),
            )
    ...
    def _derived_filename(remote_filename: str, decrypted: bool) -> str:
        base = normalize_basename(remote_filename)
    ...
    def _safe_folder_name(file_path: str) -> str:
        ...
        """
        stem = normalize_basename(file_path)
    ...
    async def send_to_temp(
        ...
        downloaded_name = normalize_basename(remote_path)
