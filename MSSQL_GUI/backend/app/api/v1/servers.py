...
from app.services.license_guard import require_not_readonly, require_can_create
import json
...
async def create_server(
    ...
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user),
    _check_readonly: None = Depends(require_not_readonly()),
    _check_limit: None = Depends(require_can_create("server")),
):
...
async def create_database(
    ...
    current_user: User = Depends(get_current_user),
    _check_readonly: None = Depends(require_not_readonly()),
    _check_limit: None = Depends(require_can_create("database")),
):
    if current_user.role not in ("superadmin", "admin", "operator"):
        raise HTTPException(status_code=403, detail="Insufficient permissions")
...
# ── Agent auto-update controls (Piano 8 / migration 0014) ────


class AutoUpdateToggle(_BaseModel):
    enabled: bool
...
async def request_update(
    ...
    """Flag a server for forced agent update at next heartbeat.
    ...
    srv.last_update_status = None
    srv.last_update_error = None
    db.add(srv)
    await db.commit()
    await db.refresh(srv)
    ...
    await _safe_audit(
        db,
        org_id=current_user.org_id, user_id=current_user.id,
        event_type=EventType.AGENT_UPDATE_REQUESTED,
        target_type="server", target_id=str(srv.id),
        description=f"Manual agent update requested for {srv.name}",
        metadata={"name": srv.name, "current_version": srv.agent_version},
        request=request,
    )
    return srv
...
async def set_auto_update(
    server_id: uuid.UUID,
    payload: AutoUpdateToggle,
    request: Request,
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user),
    _check_readonly: None = Depends(require_not_readonly()),
):
    """Enable or disable agent auto-update for a server.
    ...
    previous = srv.auto_update
    srv.auto_update = payload.enabled
    db.add(srv)
    await db.commit()
    await db.refresh(srv)
    ...
    await _safe_audit(
        db,
        org_id=current_user.org_id, user_id=current_user.id,
        event_type=EventType.AGENT_AUTO_UPDATE_CHANGED,
        target_type="server", target_id=str(srv.id),
        description=f"Auto-update {'enabled' if payload.enabled else 'disabled'} for {srv.name}",
        metadata={"name": srv.name, "previous": previous, "new": payload.enabled},
        request=request,
    )
    return srv
